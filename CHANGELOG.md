...
### ✅ Tests (Hotfix)
- Added 9 regression assertions (`BT-LinkEditText.T1.1~T1.4`, `BT-LinkEditText.1~5`) covering: linkTextInput existence in HTML (Tier 1), i18n link_text_placeholder keys (Tier 1), getLinkAttrsAtSelection text field (Tier 1), handleDoubleClick and pm-link-dblclick event (Tier 1), link command attrs.text support with replaceWith (Tier 3), pm-link-dblclick event listener in app.js (Tier 3), app.bundle.js integration (Tier 3), pm.bundle.js integration (Tier 3), link command mark-only path for unchanged text (Tier 3). 942 passing, 0 failing.
