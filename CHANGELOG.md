...
### 🐛 Bug Fixes
- **Fix task list items separated by blank lines in source after Enter in Rich Mode** — When pressing Enter to create a new task list item in Rich Mode editor, the serialized Markdown output contained blank lines between items (`- [ ] abc\n\n- [ ] 123` instead of `- [ ] abc\n- [ ] 123`). Root cause: `bullet_list` and `ordered_list` nodes in `pm-schema.js` lacked a `tight` attribute, and the parser in `pm-markdown-bridge.js` did not detect tight/loose list status from markdown-it tokens. ProseMirror-markdown's `renderList` defaults to loose (blank lines between items) when `tight` is undefined. Fix: added `tight` attribute (default `true`) to both `bullet_list` and `ordered_list` in schema; added `listIsTight` helper function and `getAttrs` in parser config to detect tight status from markdown-it token stream; added `data-tight` DOM attribute for round-trip parsing.
...
- **Prevent hyperlink navigation in Rich Mode editing** — Clicking a hyperlink in Rich Mode editor no longer triggers browser navigation. The link mark's `toDOM` now outputs `data-href` instead of `href` attribute, preventing VS Code webview from intercepting link clicks at the platform level. The `EditorView` `handleDOMEvents.click` handler calls both `event.preventDefault()` and `event.stopPropagation()` on `<a>` tag clicks, reads the URL from `data-href`, and dispatches the `pm-link-click` custom event to show the bubble menu. A `.pm-link` CSS class ensures proper link styling (color, underline, cursor) even without the native `href` attribute. The `parseDOM` also supports `a[data-href]` selector for round-trip DOM parsing.
...
- **Fix "File Updated" badge flashing after edit and not dismissible via refresh** — After editing in Rich Mode, the "File Updated" badge would flash briefly because the webview's own save (via `WorkspaceEdit`) triggered the `fileWatcher.onDidChange` event, which unconditionally sent a `fileChanged` message. Additionally, clicking the refresh button showed "File unchanged" but never dismissed the badge because `hideFileChangeBadge()` was defined but never called. Fix: (1) Added `_suppressFileChanged` flag in `customEditorProvider.ts` that is set before `applyEdit` and auto-resets after 1500ms, preventing self-triggered file change notifications; (2) Added `hideFileChangeBadge()` call in `refreshFromDisk()` after `loadDocument()` to always dismiss the badge when user explicitly refreshes.

### ✅ Tests (Hotfix)
- Added 5 regression assertions (`BT-custom-editor.T1.33~T1.34`, `BT-custom-editor.31~33`) covering: `_suppressFileChanged` flag existence in customEditorProvider.ts (Tier 1), `hideFileChangeBadge()` call in app.js refreshFromDisk (Tier 1), fileWatcher.onDidChange suppress check (Tier 3), saveFileImpl suppress-before-applyEdit ordering (Tier 3), refreshFromDisk hide-after-loadDocument ordering (Tier 3). 1001 passing, 0 failing.
