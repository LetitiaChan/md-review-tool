...
- **2026-04-18**: 🐛 Hotfix — 修复"一键AI修复时输出窗口自动弹出打断用户"。根因：`src/reviewPanel.ts` 的 `openCodeBuddyChat` 消息处理器在派发成功/失败两条路径都调用了 `outputChannel.show(true/false)` 主动弹出 Output 面板。修复：移除两处 `outputChannel.show()` 调用，OutputChannel 仅保留诊断日志写入（`[DIAG:aiChat]`、`[NEXT-STEP]`），用户可按需通过"视图 → 输出 → MD Human Review - AI Chat"手动查看；通知改为仅通过 `showInformationMessage`/`showWarningMessage` 提示。测试：新增 Suite 23 BT-aiChatOutputSilent.1~6（Tier 1/2/3，覆盖源码+编译产物的 `outputChannel.show` 关键字断言、OutputChannel 创建保留断言、[DIAG]/[NEXT-STEP] 日志写入断言、成功/失败分支静默化断言）。642 passing, 0 failing。Commit: ca21584
