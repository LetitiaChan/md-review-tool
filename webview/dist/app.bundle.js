...
`;
  ...
  function initApp() {
    ...
    let _editSnapshotBlocks = [];
    let _editSnapshotHtmls = [];
      ...
      document.getElementById("documentContent").addEventListener("click", (e) => {
        ...
        if (taskIndex >= 0) {
          ...
          if (newMarkdown !== data.rawMarkdown) {
            ...
            Store.save();
            Renderer.parseMarkdown(newMarkdown);
    ...
    async function handleSaveMd() {
      ...
      function createTurndownService() {
          ...
          replacement: function(content, node) {
            ...
            rows.forEach((row, rowIdx) => {
              ...
              const cellTexts = Array.from(cells).map((c) => {
                const htmlNormalized = tableV2Enabled ? (c.innerHTML || "").replace(/<br\s*\/?\s*>/gi, "  ") : c.innerHTML || "";
              ...
              if (rowIdx === 0) {
                const sepCells = tableV2Enabled ? alignments.map((a) => a === "left" ? ":---" : a === "center" ? ":---:" : a === "right" ? "---:" : "---") : cellTexts.map(() => "---");
        ...
        ts.addRule("imgPlaceholder", {
          filter: function(node) {
            return node.classList && node.classList.contains("img-placeholder");
          },
        ...
        ts.escape = function(s) {
          return s;
        };
        ts.keep(["kbd"]);
        return ts;
      }
      function blockHtmlToMarkdown(blockEl, turndownService2) {
        const codeBlockData = [];
        ...
        const cleanHtml = blockEl.innerHTML.replace(/<span class="annotation-indicator"[^>]*>.*?<\/span>/gi, "").replace(/<span class="annotation-fallback-marker"[^>]*>.*?<\/span>/gi, "");
        ...
        tempDiv.querySelectorAll(".diagram-edit-wrapper").forEach((wrapper, idx) => {
          const data2 = diagramEditData.find((d) => d.index === idx);
          if (data2) {
        ...
        tempDiv.querySelectorAll(".code-block").forEach((codeBlockEl, idx) => {
          const data2 = codeBlockData.find((d) => d.index === idx);
          if (data2) {
            codeBlockEl.setAttribute("data-lang", data2.dataLang);
            codeBlockEl.innerHTML = '<pre><code class="' + data2.className + '">' + data2.plainCode.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") + "</code></pre>";
          }
        });
        if (codeBlockData.length > 0 && tempDiv.querySelectorAll(".code-block").length === 0) {
          for (const data2 of codeBlockData) {
            const newCodeBlock = document.createElement("div");
            newCodeBlock.className = "code-block";
            newCodeBlock.setAttribute("data-lang", data2.dataLang);
            newCodeBlock.innerHTML = '<pre><code class="' + data2.className + '">' + data2.plainCode.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") + "</code></pre>";
        ...
        let md = turndownService2.turndown(tempDiv.innerHTML);
      ...
      function normalizeHtmlForCompare(html) {
        return html.replace(/<span class="annotation-indicator"[^>]*>.*?<\/span>/gi, "").replace(/<span class="annotation-fallback-marker"[^>]*>.*?<\/span>/gi, "").replace(/\s+/g, " ").trim();
      }
      const resultParts = [];
      ...
      {
        const snapshotUsed = /* @__PURE__ */ new Set();
      ...
      for (let i = 0; i < currentMdBlocks.length; i++) {
        ...
        const isFrontmatterBlock = alignedIdx >= 0 && alignedIdx < _editSnapshotBlocks.length && _editSnapshotBlocks[alignedIdx].startsWith("%%FRONTMATTER%%\n");
        ...
        if (snapshotHtml !== null && currentHtml === snapshotHtml && alignedIdx < _editSnapshotBlocks.length) {
          ...
          if (!hasComplexStructure && alignedIdx >= 0 && alignedIdx < _editSnapshotBlocks.length) {
            let stripMdMarkers2 = function(line) {
              return line.replace(/^(\s*)([-*+]|\d+\.)\s+(\[[ xX]\]\s*)?/, "").replace(/^#+\s+/, "").replace(/^>\s*/, "").replace(/\*\*(.+?)\*\*/g, "$1").replace(/__(.+?)__/g, "$1").replace(/\*(.+?)\*/g, "$1").replace(/_(.+?)_/g, "$1").replace(/~~(.+?)~~/g, "$1").replace(/`([^`]+)`/g, "$1").replace(/==(.+?)==/g, "$1").replace(/\^([^\^]+)\^/g, "$1").replace(/\+\+(.+?)\+\+/g, "$1").replace(/<u>(.+?)<\/u>/gi, "$1").replace(/\{color:[\w#]+(?:\([\d,.\s%]+\))?\}(.+?)\{\/color\}/g, "$1").trim();
            }, hasInlineFormatting2 = function(line) {
              return /\*\*.+?\*\*|__.+?__|(?<!\*)\*(?!\*).+?(?<!\*)\*(?!\*)|(?<!_)_(?!_).+?(?<!_)_(?!_)|~~.+?~~|`.+?`|==.+?==|\^[^\^]+\^|\+\+.+?\+\+|<u>.+?<\/u>|\{color:[\w#]+(?:\([\d,.\s%]+\))?\}.+?\{\/color\}/i.test(line);
            };
            var stripMdMarkers = stripMdMarkers2, hasInlineFormatting = hasInlineFormatting2;
            ...
            if (origLines.length === currentLines.length) {
              ...
              for (let j = 0; j < origLines.length; j++) {
                const origStripped = stripMdMarkers2(origLines[j]);
                const currStripped = currentLines[j].trim();
                if (origStripped === currStripped) {
                  patchedLines.push(origLines[j]);
                } else {
                  if (origStripped.length > 0 && currStripped.length === 0 || origStripped.length === 0 && currStripped.length > 0) {
                    canPatch = false;
                    break;
                  }
                  if (hasInlineFormatting2(origLines[j])) {
                    canPatch = false;
                    break;
                  }
          ...
          if (converted === null) {
            ...
            if (alignedIdx >= 0 && alignedIdx < _editSnapshotBlocks.length) {
              ...
              if (origLines.length === convertedLines.length) {
                ...
                let stripLineForMatch2 = function(line) {
                  return line.replace(/^(\s*)([-*+]|\d+[.)]) (\[[ xX]\]\s*)?/, "").replace(/^#+\s+/, "").replace(/^>\s*/, "").trim();
                };
                var stripLineForMatch = stripLineForMatch2;
                const origTextToIndices = {};
                for (let k = 0; k < origLines.length; k++) {
                  const text = stripLineForMatch2(origLines[k]);
                  if (!text) continue;
                  if (!origTextToIndices[text]) origTextToIndices[text] = [];
                  origTextToIndices[text].push(k);
                }
                const usedOrigIndices = /* @__PURE__ */ new Set();
                const listItemRegex = /^(\s*)([-*+]|\d+[.)]) /;
                const isListBlock = origLines.some((l) => listItemRegex.test(l));
                const matchedOrigIdx = new Array(convertedLines.length).fill(-1);
                for (let k = 0; k < convertedLines.length; k++) {
                  const convertedText = stripLineForMatch2(convertedLines[k]);
                ...
                if (isListBlock) {
                  for (let k = 0; k < convertedLines.length; k++) {
                    if (matchedOrigIdx[k] !== -1) continue;
      ...
      if (typeof window !== "undefined" && window.__DEBUG_TURNDOWN__ === true) {
        try {
          ...
          const unknownTags = /* @__PURE__ */ new Set();
      ...
      if (newContent.trim() === data.rawMarkdown.trim()) {
        ...
        setTimeout(() => updateEditStatus("", "\u7F16\u8F91\u6A21\u5F0F"), 2e3);
        return;
      }
      ...
      try {
        ...
        if (result.changed) {
          ...
          setTimeout(() => updateEditStatus("", t("notification.edit_mode")), 3e3);
